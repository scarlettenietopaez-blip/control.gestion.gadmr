...
let state = {
  pac: [], ordenanzas: [], reuniones: [], redes: [], eventos: [], apoyo: [], direccion: "TODAS", warnings: []
};
...
function pacScore(row) {
  return etapaScore(get(row, ["Etapa Actual", "Etapa_PAC", "Etapa"]), ETAPAS_PAC);
}

function ordenanzaScore(row) {
  return etapaScore(get(row, ["Etapa Actual", "Etapa_Ordenanza", "Etapa"]), ETAPAS_ORDENANZA);
}

function reunionScore(row) {
  const asistencia = get(row, ["Asistencia", "Resultado_Asistencia", "Resultado de asistencia"]);
  const exact = ASISTENCIA_PUNTAJE[asistencia];
  ...
  if (normalized.includes("no asiste")) return 0;
  return toNumber(asistencia);
}

function redesScore(row) {
  const interactions = toNumber(get(row, ["Nro de Interacciones", "N° Interacciones", "Interacciones"]));
  return interactions > 0 ? 100 : 0;
}

function eventosScore(row) {
  const personas = toNumber(get(row, ["Cantidad de personas", "Personas asistentes", "Nro de personas"]));
  return personas > 0 ? 100 : 0;
}

function apoyoScore(row) {
  const apoyo = get(row, ["Apoyo Entregado", "Cantidad", "Porcentaje_Cumplimiento"]);
  if (String(apoyo).includes("%")) return toNumber(apoyo);
  return String(apoyo || "").trim() ? 100 : 0;
}

function getDirection(row, module) {
  const fields = {
    pac: ["Dirección Responsable", "Direccion Responsable", "Direccion"],
    ordenanzas: ["Dirección Responsable", "Direccion Responsable", "Direccion"],
    reuniones: ["Direcciones Convocadas", "Dirección Convocada", "Direccion"],
    redes: ["Direcciones que comparten", "Dirección", "Direccion"],
    eventos: ["Direcciones que Asisten", "Dirección", "Direccion"],
...
function getTema(row, module) {
  const fields = {
    pac: ["Nombre del Proyecto", "Proyecto"],
    ordenanzas: ["Tema", "Nombre_Ordenanza", "Nro. Ordenanza"],
    reuniones: ["Tema Tratado", "Tema"],
    redes: ["Tema de publicación", "Publicación"],
...
function getDate(row, module) {
  const fields = {
    pac: ["Fecha límite", "Fecha_Limite", "Fecha de inicio"],
    ordenanzas: ["Fecha de finalización", "Fecha_Prevista", "Fecha de asignación"],
    reuniones: ["Fecha"],
    redes: ["Fecha de Publicación", "Fecha"],
    eventos: ["Fecha"],
    apoyo: ["Fecha"]
  };
...
function isDelayed(row, module) {
  const estado = normalize(get(row, ["Estado", "Estado_General", "Etapa Actual"]));
  if (estado.includes("atrasado") || estado.includes("critico") || estado.includes("vencido")) return true;
  if (estado.includes("cumplido") || estado.includes("finalizado") || estado.includes("aprobada") || estado.includes("cerrado")) return false;
  const fecha = getDate(row, module);
...
function getDirections() {
  ...
  for (const module of ["pac", "ordenanzas", "reuniones", "redes", "eventos", "apoyo"]) {
    state[module].forEach(row => {
      const direction = getDirection(row, module);
...
function filteredRows(rows, module) {
  ...
  return rows.filter(row => getDirection(row, module) === state.direccion);
}

function groupByDirection(rows, module, scoreFunction) {
  return rows.reduce((acc, row) => {
    const key = getDirection(row, module) || "Sin dirección";
    if (!acc[key]) acc[key] = [];
...
function freshnessScore(rows) {
  const dates = rows
    .map(r => get(r, ["Última Actualización", "Ultima_Actualizacion", "Fecha de Publicación", "Fecha", "Fecha límite", "Fecha de finalización"]))
    .filter(Boolean)
...
function calculateDirectionScores() {
  ...
  return directions.map(direction => {
    const pacRows = state.pac.filter(r => getDirection(r, "pac") === direction);
    const ordRows = state.ordenanzas.filter(r => getDirection(r, "ordenanzas") === direction);
    const reuRows = state.reuniones.filter(r => getDirection(r, "reuniones") === direction);
    const redRows = state.redes.filter(r => getDirection(r, "redes") === direction);
    const eveRows = state.eventos.filter(r => getDirection(r, "eventos") === direction);
    const apoRows = state.apoyo.filter(r => getDirection(r, "apoyo") === direction);
...
function makeAlerts() {
  ...
  filteredRows(state.pac, "pac").forEach(r => {
    if (isDelayed(r, "pac") || pacScore(r) === 0) alerts.push({ modulo: "PAC", direccion: getDirection(r, "pac"), tema: getTema(r, "pac"), problema: isDelayed(r, "pac") ? "Proceso vencido o atrasado" : "Proceso no iniciado", prioridad: "Media" });
  });

  filteredRows(state.ordenanzas, "ordenanzas").forEach(r => {
    if (isDelayed(r, "ordenanzas") || ordenanzaScore(r) === 0) alerts.push({ modulo: "Ordenanza", direccion: getDirection(r, "ordenanzas"), tema: getTema(r, "ordenanzas"), problema: isDelayed(r, "ordenanzas") ? "Ordenanza vencida o atrasada" : "Ordenanza no iniciada", prioridad: "Media" });
  });
...
function renderKPIs() {
  ...
  const interactions = red.reduce((sum, row) => sum + toNumber(get(row, ["Nro de Interacciones", "N° Interacciones", "Interacciones"])), 0);
  const asistentes = eve.reduce((sum, row) => sum + toNumber(get(row, ["Cantidad de personas", "Personas asistentes", "Nro de personas"])), 0);
  ...
  const kpis = [
    ...
    { label: "Interacciones en redes", value: interactions, note: `${red.length} publicaciones registradas` },
    { label: "Asistencia a eventos", value: asistentes, note: `${eve.length} eventos registrados` },
